...
class LineEntity(TextEntity):
    ...
    def render(self, parent=0, attributes_group=0):
        self.post_render_queue = list()
        for item in self:
            with dpg.group(horizontal=True, parent=parent) as group:
                # Retrieve attributes for the current item
                attributes = item.get_all_attributes()
                list_attributes = self.get_attributes_by_type(attributes, List)

                if list_attributes:
                    # Assuming only one List attribute per line
                    list_attr = list_attributes[0]
                    # Determine indentation based on depth
                    indentation = "    " * list_attr.depth
                    # Determine the list marker
                    if list_attr.ordered:
                        marker = f"{list_attr.index}."
                    else:
                        marker = "-"

                    # Create a text entity for the marker
                    marker_text = StrEntity(f"{indentation}{marker} ")
                    # Set attributes (excluding List to prevent recursion)
                    marker_text.set_attributes([attr for attr in attributes if not isinstance(attr, List)])
                    # Render the marker
                    marker_text.render(parent=group, attributes_group=attributes_group)

                # Render the attributes for the item (excluding List)
                self.render_attributes(item, parent=group, attributes_group=attributes_group)
                # Render the item content
                item.render(parent=group, attributes_group=attributes_group, max_text_height=item.get_height())
    ...
    def render_attributes(self, text_entity: TextEntity, parent=0, attributes_group=0):
        attributes = text_entity.get_all_attributes()
        # Exclude List attributes to prevent duplicate rendering
        attributes = [attr for attr in attributes if not isinstance(attr, List)]
